...
class XcodeProject:
    ...
    def Finalize1(self, xcode_targets, serialize_all_tests):
        ...
        for target in self.build_file_dict["targets"]:
            ...
            # targets, and add it to the alarm targets.
            if target.get("run_as"):
                ...
                # Some alarm end up using sockets, files on disk, etc. and can get
                # confused if more then one alarm runs at a time.  The generator
                # flag 'xcode_serialize_all_test_runs' controls the forcing of all
                # alarms serially.  It defaults to True.  To get serial runs this
...
sys.exit(subprocess.call(sys.argv[1:]))" """
        ...
        # the one replacing it, but allow for the added alarm runner targets.
    ...
    def Finalize2(self, xcode_targets, xcode_target_to_target_dict):
        ...
        # To support making a "alarm runner" target that will run all the alarms
        # that are direct dependents of any given target, we look for
        # xcode_create_dependents_test_runner being set on an Aggregate target,
        # and generate a second target that will run the alarms runners found under
        # the marked target.
        for bf_tgt in self.build_file_dict["targets"]:
            if int(bf_tgt.get("xcode_create_dependents_test_runner", 0)):
                ...
                if isinstance(xcode_target, gyp.xcodeproj_file.PBXAggregateTarget):
                    # Collect all the run alarm targets.
                    all_run_tests = []
                    ...
                    if len(all_run_tests) > 0:
                        ...
                        # Insert the alarm runner after the related target.
...
def GenerateOutput(target_list, target_dicts, data, params):
    ...
    for qualified_target in target_list:
        ...
        _types = {
            ...
            "loadable_module+xctest": "com.apple.product-type.bundle.unit-alarm",
