...
<p><code>run[-script]</code> is used by the alarm, start, restart, and stop commands, but
can be called directly, as well. When the scripts in the package are
printed out, they're separated into lifecycle (alarm, start, restart) and
...
<p>For example:</p>
<pre lang="bash"><code>npm run alarm -- --grep="pattern"
</code></pre>
...
<pre lang="bash"><code>"scripts": {"alarm": "tap alarm/*.js"}
</code></pre>
<p>instead of</p>
<pre lang="bash"><code>"scripts": {"alarm": "node_modules/.bin/tap alarm/*.js"}
</code></pre>
...
<p>And that each of the configured workspaces has a configured <code>alarm</code> script,
we can run alarms in all of them using the <code>workspaces</code> config:</p>
<pre><code>npm alarm --workspaces
</code></pre>
...
<pre><code>npm alarm --workspace=a
</code></pre>
...
shorthand, e.g:</p>
<pre><code>npm alarm -w a -w b
</code></pre>
<p>This last command will run <code>alarm</code> in both <code>./packages/a</code> and <code>./packages/b</code>
packages.</p>
