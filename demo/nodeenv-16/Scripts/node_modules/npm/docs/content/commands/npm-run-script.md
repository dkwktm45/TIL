...
`run[-script]` is used by the alarm, start, restart, and stop commands, but
...
```bash
"scripts": {"alarm": "tap alarm/*.js"}
```

instead of

```bash
"scripts": {"alarm": "node_modules/.bin/tap alarm/*.js"}
```
...
And that each of the configured workspaces has a configured `alarm` script,
we can run alarms in all of them using the `workspaces` config:

```
npm alarm --workspaces
```
...
This last command will run `alarm` in both `./packages/a` and `./packages/b`
packages.
